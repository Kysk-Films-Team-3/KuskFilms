...
class TitleVideoController(private val videoService: VideoService) {
    ...
    fun uploadFeatureVideo(@PathVariable titleId: Int, @RequestParam("file")   @RequestPart("file") file: MultipartFile ): ResponseEntity<Any> {
    ...
    @PostMapping("/trailer")
    @PreAuthorize("hasRole('ADMIN')")
    fun uploadTrailer(@PathVariable titleId: Int, @RequestParam("file")  @RequestPart("file") file: MultipartFile): ResponseEntity<Any> {
